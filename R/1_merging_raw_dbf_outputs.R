...
file_list<-file_list[lapply(file_list,function(x) length(grep("*.csv*",x,value=FALSE))) == 1]
...
droplevels(dataset)
unique(dataset$ISO3)
unique(dataset$ISO3_edt)

#make iso3 column same as iso3_edt
dataset$ISO3<-dataset$ISO3_edt
#remove iso3_edt column
dataset$ISO3_edt<-NULL


names(dataset)
temp_dataset <- read.csv("merged_aggsr_admin1.csv")
#temp_dataset <- read.dbf(file)
names(temp_dataset)
#temp_dataset<-temp_dataset[,c("WDPAID","DESIG_ENG","DESIG_TYPE","ISO3_edt","GEOandUNEP","ISO3_1","GEOandUN_1","AREA_GEO","INSIDE_X","INSIDE_Y")]
#temp_dataset<-temp_dataset[,c("WDPAID","DESIG_ENG","DESIG_TYPE","ISO3_edt","GEOandUNEP","ISO3_1","GEOandUNEP_1","AREA_GEO","INSIDE_X","INSIDE_Y")]
...
head(formatC(x, format="f", digits=8))
...
write.csv(dataset,paste0(out.folder,"/","merged_aggsr_admin2.csv"), na="0",row.names=FALSE )
