...
setwd(in.folder)
getwd()
dataset<-read.csv("merged_aggsr_admin.csv")#header=TRUE,sep=",")
head(dataset)
tmp<-dataset
...
reshapedTypeAll$countSum <- rowSums( reshapedType[,6:length(names(reshapedTypeAll))])
